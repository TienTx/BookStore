...
$(document).ready(function () {
    ...
    $('.userOption #cardpoint').click(function () {
        window.location = contextPath + "/cardpoint.html";
    });
...
//        $('#odcart').removeClass("hide");
    ...
    $("#checkcp").submit(function (e) {
        $.ajax({
            type: "POST",
            url: contextPath + "/cardpoint/checkcardpoint",
            header: {
                Accept: "application/json; charset=utf-8",
                "Content-Type": "application/json; charset=utf-8"
            },
            data: $("#checkcp").serialize(),
            success: function (response) {
                if (response !== null && response !== "fail") {
                    $('#cpoint').html(response.split("#")[0]);
                    $('#cemail').html(response.split("#")[1]);
                    $('#cphone').html(response.split("#")[2]);
                    $('#wrong').attr("class", "hide");
                } else {
                    $('#wrong').removeClass("hide");
                    $('#cpoint').html("");
